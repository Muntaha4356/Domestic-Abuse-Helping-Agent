...
import { sendVoiceMessage } from "@/functions/sendVoiceMessage";
...
export function ChatWindow({ onLeave }: Props) {
  ...
  const handleSendVoice = async (base64Audio: string, mimeType: string, clientTranscript: string) => {
    if (isTyping) return;
    const userMsg: ChatMessage = {
      id: `u-${Date.now()}`,
      role: "user",
    ...
    try {
      ...
      if (result.transcription) {
        setMessages((m) =>
          m.map((msg) =>
            msg.id === userMsg.id ? { ...msg, content: result.transcription } : msg
          )
        );
      }
